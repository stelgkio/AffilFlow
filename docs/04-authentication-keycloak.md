...
3. **Roles** — JWT claim `roles` is an array of strings (`affiliate` default, `merchant` for merchants). Routes use `RequireRoles("merchant")` where needed.
4. **Company onboarding** — `POST /api/v1/onboarding/company` (authenticated) creates a campain, subscribes the free plan, sets `users.campain_id`, and promotes the user to `merchant`.
...
| `AUTH_BOOTSTRAP_ADMIN_EMAIL` | Optional: first login with this email becomes `merchant` |
