...
Each **merchant** using AffilFlow belongs to an **organization** with a **subscription** and an **invite cap** (how many affiliates they may onboard). Defaults: **free = 3 invites**; paid **€10 / €20 / €50** per month with higher caps (see [12-platform-subscriptions-billing.md](12-platform-subscriptions-billing.md)).

```mermaid
erDiagram
  organizations ||--o{ users : "members"
  organizations ||--o{ affiliates : "owns"
...
Affiliate and order rows should be **scoped by `organization_id`** (add FK where missing in implementation).

### Affiliate invites and discovery (see [13](13-affiliate-onboarding-and-discovery.md))

```mermaid
erDiagram
  organizations ||--o{ affiliate_invites : "issues"
  affiliate_invites {
    uuid id PK
    uuid organization_id FK
    text email "nullable"
...
Optional **organization** columns: `slug`, `discovery_enabled`, `approval_mode` for the public directory and apply flows.
...
| Table | Purpose |
|-------|---------|
| **organizations** | Paying tenant; subscription and invite limits apply here |
| **subscription_plans** | Plan keys, EUR price, **max_invites**, Stripe price ids |
| **subscriptions** | Active Stripe subscription per organization |
| **users** | Mirror of Keycloak subjects; may belong to an **organization** |
| **affiliates** | Business entity: code, rate, status; links to `users` and **organization** |
