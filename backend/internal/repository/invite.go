...
func (r *InviteRepository) Insert(ctx context.Context, orgID uuid.UUID, email *string, tokenHash string, expiresAt time.Time, createdBy *string) (uuid.UUID, error) {
	const q = `
		INSERT INTO affiliate_invites (organization_id, email, token_hash, expires_at, status, created_by_user_id)
		VALUES ($1, $2, $3, $4, 'pending', $5)
		RETURNING id
	`
	var id uuid.UUID
	err := r.pool.QueryRow(ctx, q, orgID, email, tokenHash, expiresAt, createdBy).Scan(&id)
	return id, err
}
...
func (r *InviteRepository) GetPendingByHash(ctx context.Context, tokenHash string) (*models.AffiliateInvite, error) {
	const q = `
		SELECT id, organization_id, email, token_hash, expires_at, status, created_by_user_id, created_at
		FROM affiliate_invites
		WHERE token_hash = $1 AND status = 'pending' AND expires_at > now()
	`
	var m models.AffiliateInvite
	err := r.pool.QueryRow(ctx, q, tokenHash).Scan(
		&m.ID, &m.OrganizationID, &m.Email, &m.TokenHash, &m.ExpiresAt, &m.Status, &m.CreatedByUserID, &m.CreatedAt,
	)
